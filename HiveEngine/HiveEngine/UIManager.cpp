...
namespace Hive
{
	...
	void UIManager::update(float delta) {
		...
		player_unit = Unit::get_component(player_unit_handle);
		//cout << Unit::get_component(player_unit_handle)->get_vitals().hp << endl;
		temp_hp_bar.width = temp_hp_bar.orig_width * (player_unit->get_vitals().hp / player_unit->get_max_vitals().hp);
